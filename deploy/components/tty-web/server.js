...
app.get('/api/md-files', (req, res) => {
  ...
  if (project) {
    const projDir = '/root/projects/' + project;
    try {
...
app.get('/api/md-file', (req, res) => {
  ...
  if (!id.startsWith('/root/')) return res.status(403).send('forbidden');
...
app.post('/api/md-file', express.json(), (req, res) => {
  ...
  if (!id.startsWith('/root/')) return res.status(403).json({ error: 'forbidden' });
...
function triggerContextUpdate(sessionName) {
  spawn('/bin/bash', ['/opt/agent-stack/heartbeat.sh', sessionName], {
...
app.post('/api/delete', express.json(), (req, res) => {
  ...
  try {
    ...
    if (content.trim()) {
      ...
      spawn('/bin/bash', ['/opt/agent-stack/heartbeat.sh', sessionName, tmp], {
        detached: true, stdio: ['ignore', 'ignore', 'ignore']
      }).unref();
    }
  } catch(_) {}
...
app.post('/api/slack-last', express.json(), (req, res) => {
  ...
  try {
    ...
    const child = spawn('python3', [(process.env.AGENT_STACK_DIR || '/opt/agent-stack') + '/notify.py', msg], { timeout: 10000 });
